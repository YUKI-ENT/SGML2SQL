...
### LLM処理共通の更新日判定（23→24、31→33、41→43）
...
- 更新候補でも、文章ハッシュが同じなら24・33・43の保存済み成功結果を再利用します。未処理・失敗・要確認のLLM処理は更新日で除外せず、各コマンドの実行モードに従います。モデルやプロンプトの変更も従来どおり扱います。

成功時の比較基準は、各ブロックテーブル名に `_source_state` を付けた永続テーブルに保存します。一時ファイルは使いません。ブロックの更新と基準日の保存は同じトランザクションで確定します。23は全件TRUNCATEせず、変更文書のブロックだけを置換し、全件実行時に原本から消えた文書を除去します。

**既存環境からの移行:** 実行中の24・33・43が終わってから、次の21による再取込の前に、現在のrawdataに対して23・31・41を実行すると、旧データの比較基準を保存できます（これら3本はLLMを呼びません）。旧rawdataでは `raw_xml_path` に含まれる配布フォルダの日付を利用し、`updated_at` をダウンロード日の代わりにはしません。基準を保存せず新データへ入れ替えた場合、初回は安全のため再抽出し、既存LLMキャッシュを利用します。日付判定が実行中の24の対象を途中で減らすことはありません。

次回以降は、`DI_folder` を今回の配布フォルダへ変更し、21→23→24、21→31→32→33→34、21→41→42→43→44の順に実行します。CSVの日付は日単位なので、同日中の差し替えは日付一致だけでは検出できません。必要時は23・31・41の `--force`、または `sgml_use_file_list_dates: false` で再抽出できます。1か月の猶予も、それを超える公開遅延を保証するものではありません。
